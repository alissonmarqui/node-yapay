    ...
    request.post(options, (err, response, body) => {
        if (err) {
            ...
            if (body.message_response.message === 'error') {
                ...
                this.code = body.data_response.authorization.code;                
                return cb(false, body.data_response);
            }
        }
    })
...
yapay.prototype.generateAccessToken = function(consumer_key, consumer_secret, cb) {
    const options = {
...
yapay.prototype.generateAccessTokenWithCode = function(consumer_key, consumer_secret, code, cb) {
    const options = {
        url: this.url + '/v1/authorizations/access_token',
        json: {
            consumer_key: consumer_key,
